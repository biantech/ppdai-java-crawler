...
public class HttpUnitPostDemo {
    //https://blog.csdn.net/jethai/article/details/52345438

    static  Logger logger = LoggerFactory.getLogger(HttpUnitPostDemo.class);
    /** 无法实现js页面的自动跳转，HtmlUnit可以      */
    public static void main(String[] args) throws Exception {
        // 这是一个测试，也是为了让大家看的更清楚，请暂时抛开代码规范性，不要纠结于我多建了一个局部变量等
        // 得到认证https的浏览器对象
        HttpUnitPostDemo postDemo = new HttpUnitPostDemo();
        postDemo.doPost();
    }
    ...
    private void showResponseInfo(HttpResponse hr) throws ParseException, IOException {
        logger.info("响应状态行信息：" + hr.getStatusLine());
        logger.info("---------------------------------------------------------------");
        logger.info("响应头信息：");
        ...
        logger.info("---------------------------------------------------------------");
        logger.info("响应正文：");
        logger.info(EntityUtils.toString(hr.getEntity()));
 /*   <body><script>
 location.href="http://passport.tianya.cn:80/online/loginSuccess.jsp?fowardurl=http%3A%2F%2Fwww.tianya.cn%2F110486326&userthird=®Orlogin=%E7%99%BB%E5%BD%95%E4%B8%AD......&t=1458895519504&k=06d41f547cd05fb5dea1590a60e1ec98&c=669767baea73097dde58423fac777138";
    ...
    // 得到一个认证https链接的HttpClient对象（因为我们将要的天涯登录是Https的）       // 具体是如何工作的我们后面会提到的
    private HttpClient getSSLInsecureClient() throws Exception {
    ...
    private HttpPost getPostComment(){
        HttpPost post = new HttpPost("https://localhost:9080/findComment");
        // 首先我们初始化请求头
        post.addHeader("Content-Type","application/json");
        // 然后我们填入我们想要传递的表单参数
        // 我们可以先建立一个List，之后通过post.setEntity方法传入即可
        List<NameValuePair> paramsList = new ArrayList<>();
        paramsList.add(new BasicNameValuePair("blogId", "1"));
        paramsList.add(new BasicNameValuePair("pageSize", "10"));
        // 将这个参数list设置到post中
